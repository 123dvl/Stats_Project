...
def stem_and_leaf_plot(numbers):
    ...
    for stem, leaves in sorted(stem_leaf_dict.items()):
        stem_str = f"{round(stem)} | "
        leaf_str = " ".join(map(str, sorted(leaves)))
        print(stem_str.ljust(3), leaf_str)
...
def calculate_standard_deviation_proportions(numbers):
    n = len(numbers)
    mean = sum(numbers) / n
    std_dev = (sum((x - mean) ** 2 for x in numbers) / n) ** 0.5
    print(f"STD: {std_dev}")

    count_within_1_std_dev = 0
    count_within_2_std_dev = 0
    count_within_3_std_dev = 0

    for x in numbers:
        if mean - std_dev <= x <= mean + std_dev:
            count_within_1_std_dev += 1
        if mean - 2 * std_dev <= x <= mean + 2 * std_dev:
            count_within_2_std_dev += 1
        if mean - 3 * std_dev <= x <= mean + 3 * std_dev:
            count_within_3_std_dev += 1

    proportion_within_1_std_dev = count_within_1_std_dev / n
    proportion_within_2_std_dev = count_within_2_std_dev / n
    proportion_within_3_std_dev = count_within_3_std_dev / n

    print("\n-------Standard Deviation Proportion-------\n")

    print(f"Proportion of observations within 1 standard deviation from the mean: {proportion_within_1_std_dev:.3f}")
    print(f"Proportion of observations within 2 standard deviations from the mean: {proportion_within_2_std_dev:.3f}")
    print(f"Proportion of observations within 3 standard deviations from the mean: {proportion_within_3_std_dev:.3f}")
...
if __name__ == "__main__":
    ...
    calculate_standard_deviation_proportions(numbers)