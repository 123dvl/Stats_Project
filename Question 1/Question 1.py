...
def proportion_less_than(numbers, threshold):
    count_less = sum(1 for num in numbers if num < threshold)
    return count_less / len(numbers)


def proportion_above(numbers, threshold):
    above_count = sum(1 for num in numbers if num >= threshold)
    total_count = len(numbers)
    proportion = above_count / total_count
    return proportion


def proportion_between(numbers, lower, upper):
    count = 0
    for num in numbers:
...
if __name__ == "__main__":
    ...
    num_intervals = 12  # Set the predetermined class interval here
    ...
    prop_less_than_x = proportion_less_than(numbers, 6)
    print(f"The proportion of observations less than 6 is: {prop_less_than_x:.3f}")
    proportion = proportion_above(numbers, 10)
    print(f"The proportion of numbers at least 10 is: {proportion:.3f}")
    prop = proportion_between(numbers, 4, 8)
    print(f"The proportion of observations between 4 and 8 is: {prop:.3f}")